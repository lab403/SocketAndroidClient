...
public class Tab_music extends Activity {
	Socket s = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tab_music);// 音樂畫面
		//this.OpenFile();
	}
	...
	private void OpenFile(){
		try {
			...
			dout.writeUTF("MRCode_Show_Music");// 向伺服器發送訊息
			//切換至檔案清單畫面
			...
			if(in.equalsIgnoreCase("NoFile")){
				...
				intent.setClass(Tab_music.this, LV_Music.class);
				intent.putExtra("File", in.split("//s"));
				startActivityForResult(intent, 9652);
			}
		} catch (Exception e) {
		...
		if(requestCode==9652){
			if(resultCode==RESULT_OK){
				try {
					...
					dout = new DataOutputStream(s.getOutputStream());// 得到輸出串流
					din = new DataInputStream(s.getInputStream());// 得到輸入串流
					dout.writeUTF("MRCode_Run_Music//s"+data.getCharSequenceExtra("File"));// 向伺服器發送訊息
					String in = din.readUTF();
	...
	public void onClick(View v) {// 依照所案按鍵發送命令碼
		switch (v.getId()) {
		case R.id.music_power_off: // 關閉播放器
			RunCommend("MRCode_WMP_00");
			break;
		case R.id.music_itme_up: // 上一首
			RunCommend("MRCode_WMP_12");
			break;
		case R.id.music_item_play: // 繼續或暫停
			RunCommend("MRCode_WMP_10");
			break;
		case R.id.music_item_down: // 下一首
			RunCommend("MRCode_WMP_13");
			break;
		case R.id.music_itme_beforePlay: // 慢速播放
			RunCommend("MRCode_WMP_18");
			break;
		case R.id.music_item_stopPlay: // 停止播放
			RunCommend("MRCode_WMP_19");
			break;
		case R.id.music_item_afterPlay: // 快速播放
			RunCommend("MRCode_WMP_17");
			break;
		case R.id.music_audio_add: // 音量增加
			RunCommend("MRCode_WMP_15");
			break;
		case R.id.music_audio_cut: // 音量減少
			RunCommend("MRCode_WMP_16");
			break;
		case R.id.music_audio_off: // 靜音
			RunCommend("MRCode_WMP_14");
			break;
		case R.id.music_random: // 隨機播放
			RunCommend("MRCode_WMP_01");
			break;
		case R.id.music_repeat: // 重複播放
			RunCommend("MRCode_WMP_02");
			break;
		case R.id.music_list: // 要求音樂資料夾底下的檔案
			this.OpenFile();
			break;
		}
	}
